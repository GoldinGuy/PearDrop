...
class SenderPacket {
  ...
  SenderPacket(String filename, String mimetype, int data_len) {
    ...
    ptr = native_senderpacket_create(cfilename, cmimetype, data_len as Uint64);
    assert(ptr != nullptr, 'Failed to create SenderPacket');
    free(cfilename);
    free(cmimetype);
  }
  /// Reads a [SenderPacket] from the given buffer.
  SenderPacket.read(List<Uint8> buffer) {
    var cbuffer = cbcopy(buffer);
    ptr = native_senderpacket_read(cbuffer, buffer.length as Uint64);
  ...
  int get data_len {
    ...
    assert((res as int) == 0, 'Failed to get data length of SenderPacket');
    var value = out.value;
    free(out);
    return value;
  }
  ...
  List<Uint8> write() {
    ...
    assert((res as int) == 0, 'Failed to write SenderPacket');
