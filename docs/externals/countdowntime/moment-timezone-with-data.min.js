!(function (c, M) {
	...
	"function" == typeof define && define.amd
		...
		? (module.exports = M(require("moment")))
		: M(c.moment);
})(this, function (c) {
	"use strict";
	function M(c) {
	...
	function z(c) {
		var z = 0,
			...
			n = 0,
			a = 1;
		for (45 === c.charCodeAt(0) && ((z = 1), (a = -1)), z; z < b.length; z++)
			n = 60 * n + M(b.charCodeAt(z));
		for (z = 0; z < o.length; z++) (p /= 60), (n += M(o.charCodeAt(z)) * p);
		return n * a;
	}
	function A(c) {
	...
	function L(c, M) {
		for (var z, A; (A = 6e4 * (((M.at - c.at) / 12e4) | 0)); )
			(z = new a(new Date(c.at + A))).offset === c.offset ? (c = z) : (M = z);
		return c;
	}
	function q() {
		var c,
			...
			o = [b];
		for (z = 1; z < 48; z++)
			(M = new a(new Date(A, z, 1))).offset !== b.offset &&
				((c = L(b, M)), o.push(c), o.push(new a(new Date(c.at + 6e4)))),
				(b = M);
		for (z = 0; z < 4; z++)
			o.push(new a(new Date(A + z, 0, 1))),
				o.push(new a(new Date(A + z, 6, 1)));
		return o;
	}
	function N(c, M) {
	...
	function i(c, M) {
		var z, b;
		for (A(M), z = 0; z < M.length; z++)
			(b = M[z]), (m[b] = m[b] || {}), (m[b][c] = !0);
	}
	function W(c) {
		var M,
			z,
		...
		for (M in o) o.hasOwnProperty(M) && p.push(S[M]);
		return p;
	}
	function d() {
		try {
		...
		for (A = 0; A < n.length; A++) {
			for (z = new O(f(n[A]), p), b = 0; b < p; b++) z.scoreOffsetAt(o[b]);
			a.push(z);
		}
		return a.sort(N), a.length > 0 ? a[0].zone.name : void 0;
	}
	function e(c) {
	...
	function f(c, M) {
		...
		return A instanceof n
			...
			? ((A = new n(A)), (E[c] = A), A)
			: R[c] && M !== f && (z = f(R[c], f))
			? ((A = E[c] = new n())._set(z), (A.name = S[c]), A)
			: null;
	}
	function B(c) {
		var M, z, A, b;
	...
	function r(c) {
		X(c.zones), B(c.links), (t.dataVersion = c.version);
	}
	function T(c) {
		return (
			T.didShowError ||
	...
	function t(M) {
		...
		return (
			b && !c.isMoment(M) && l(o) && o.add(b.parse(o), "minutes"), o.tz(A), o
		);
	}
	function s(c) {
		return function () {
	...
	(P < 2 || (2 === P && g < 6)) &&
		...
		(n.prototype = {
			...
			_index: function (c) {
				var M,
					z = +c,
					A = this.untils;
				for (M = 0; M < A.length; M++) if (z < A[M]) return M;
			},
			parse: function (c) {
				var M,
					z,
				...
				for (b = 0; b < a; b++)
					if (
						...
						(z = p[b + 1]),
						(A = p[b ? b - 1 : b]),
						M < z && t.moveAmbiguousForward
							? (M = z)
							: M > A && t.moveInvalidForward && (M = A),
						o < n[b] - 6e4 * M)
					)
						return p[b];
				return p[a];
			...
			offset: function (c) {
				return (
					u("zone.offset has been deprecated in favor of zone.utcOffset"),
		...
		(O.prototype.scoreOffsetAt = function (c) {
		...
		(t.names = function () {
			var c,
				M = [];
			for (c in S)
				S.hasOwnProperty(c) && (E[c] || E[R[c]]) && S[c] && M.push(S[c]);
			return M.sort();
		}),
		(t.Zone = n),
		(t.unpack = p),
	...
	(c.tz = t),
		...
		(c.updateOffset = function (M, z) {
			...
			void 0 === M._z &&
				(b &&
					l(M) &&
					!M._isUTC &&
					((M._d = c.utc(M._a)._d), M.utc().add(b.parse(M), "minutes")),
				(M._z = b)),
				M._z &&
					((A = M._z.utcOffset(M)),
					Math.abs(A) < 16 && (A /= 60),
					void 0 !== M.utcOffset ? M.utcOffset(-A, z) : M.zone(A, z));
		}),
		(h.tz = function (M, z) {
			return M
				? ((this._z = f(M)),
				  this._z
						? c.updateOffset(this, z)
						: u(
		...
		(h.zoneName = s(h.zoneName)),
		(h.zoneAbbr = s(h.zoneAbbr)),
		(h.utc = (function (c) {
			return function () {
